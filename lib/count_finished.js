...
//the expecation is that this will be called in a mongoose query looking for all activities of a specific user, so the "activities" parameters is an array of activities owned by a specific user
...
const countFinished = (activities) => {
  ...
  completedActivities.forEach(
    (activity) => (completedCounts[activity.type] += 1)
  );
