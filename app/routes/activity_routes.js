  ...
  Activity.find({ owner: req.params.userId })
    ...
    .then((activities) => {
      ...
      const publicActivities = activities.filter(
        (activity) => activity.private === false
      );
      //get completedCounts
      const completedCounts = countFinished(activities);
      //return badges
...
router.get("/activities/:id", requireToken, (req, res, next) => {
  Activity.findById(req.params.id)
    ...
    .then(handle404)
    .then((activity) => {
      let privateViewableNotes = activity.notes.filter(
        (noteObj) => noteObj.owner.id == req.user.id && noteObj.private === true
      );
      ...
      res
        ...
        .json({
          ...
          privateViewableNotes: privateViewableNotes,
        });
    })
    .catch(next);
});
